function reloadLightbox() {
    ...
    allPhotosDiv.forEach(Div => { // on boucle sur toutes les div photos, on récup les attributs
        const srcPhoto = Div.dataset.src;
        const srcCat = Div.dataset.cat;
        const srcRef = Div.dataset.ref;

        photosArray.push({ // on ajoute au tableau les attributs de la photo précedemment récup
            'id': index,
            'img': srcPhoto,
            'cat': srcCat,
            'ref': srcRef
    ...
    iconLightbox.forEach((icon, id) => { //on boucle sur le bouton d'ouverture de le lightbox
        icon.addEventListener('click', function () {
            ...
            if (targetPhoto.id === targetPhoto.length - 1) { // si c'est la derniere phto, alors on enleve le bouton next (17-1)
                iconnext.classList.add('hidden');
            };
                // on change la photo, cat et ref 
    ...
    iconprevious.addEventListener('click', () => {
        iconnext.classList.remove('hidden'); //on remet le bouton précé
        const currentIdPhoto = targetPhoto.id - 1; // on récup l'id de la photo actuelle
        const previousPhoto = photosArray[currentIdPhoto - 1]; // on récup la photo précé dans le tableau qui comprend toutes les photos



        if (previousPhoto) { // si il exsite une photo précé, on redéfinit la photo actuelle par la photo précé
